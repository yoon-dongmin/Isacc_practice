...
class PrimitiveAction(object):
    ...
    def reciprocating_motion(self, direction, distance, avoid_collision=True, reference="world"):
        ...
        if self.self_play:
            ...
            input('reciprocating_motion: {}'.format(fraction))
            # rospy.loginfo('reciprocating_motion: {}'.format(fraction))
        execution_time = time.time() - start
    ...
    def circular_motion(self, normal_direction, direction, radius, start_angle, end_angle, avoid_collision):
        ...
        if self.self_play:
            ...
            input('circular_motion: {}'.format(fraction))
    ...
    def _reciprocating_motion(self, distance, avoid_collision=True):
        try:
            ...
            plan = []
            return plan
    def grasp(self,grasp_size=0.04):
        try:
            robot = self.robot 
            hand_group = self.hand_group 
            eef_link = self.eef_link 
...
def main():
    ...
    #print(pre_pose)
    input() 
    pose_test.move_to(pre_pose)
    input() 
    pose_test.linear_motion([0, 0, 0.1], True, "eef")
    input() 
    pose_test.grasp(grasp_size=0.02)
    input() 
    pose_test.linear_motion([0, 0, post_dist], True)

    #place
    # input() 
    # a = pose_test.get_cur_pose()
    # print(a)
    pose_goal.position.x = 0.3418650509445885
    pose_goal.position.y = 0.35937840330179416
    pose_goal.position.z = 0.7769127410491126

    pose_goal.orientation.x = 0.9244452018996354
    pose_goal.orientation.y = -0.3809807945467783
    pose_goal.orientation.z = -0.01320739798268974
    pose_goal.orientation.w = 0.008959213680601113
    input() 
    ...
    #pose_test.linear_motion([0,0.5,0], True, reference="eef")
    #input() 
    #pose_test.circular_motion('Z', 'CW', 0.1, 0, 360, False)
    #input() 
    #pose_test.reciprocating_motion("Y",0.1,False)
